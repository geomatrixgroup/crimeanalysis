...
@Injectable()
export class DldtService {

  constructor() { }
  
  diyDldt(prototype,element:Element):[HTMLElement,HTMLElement]{
    //对控制区增添内容
    let bounding=element.getBoundingClientRect();
    let dldtControl=this.diyControl(prototype[0],bounding);
      //给关闭按钮注册事件
      //给最小化按钮注册事件
/*     this.closeEvent(<HTMLElement>dldtControl.children[0]);
    ...
    //对图表区增添内容
    let dldtChart=this.diyChart(prototype[1]);
    return [dldtControl, dldtChart];
  }
 ...
 /*  private closeEvent(close:HTMLElement){
  ...
  private diyControl(control:HTMLElement,bounding):HTMLElement{
    control.setAttribute('style', 'position: absolute;z-index: 1005;display:inline-block;background:white;border-radius: 4px;box-shadow: 2px 5px 5px rgba(0,0,0,.3);');
    ...
    content.appendChild(document.createTextNode('地理底图'));
