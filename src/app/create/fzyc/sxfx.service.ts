...
@Injectable()
export class SxfxService {

  constructor() { }
  
  diySxfx(prototype,element:Element):[HTMLElement,HTMLElement]{
    //对控制区增添内容
    let bounding=element.getBoundingClientRect();
    let  sxfxControl=this.diyControl(prototype[0],bounding);
      //给关闭按钮注册事件
      //给最小化按钮注册事件
/*     this.closeEvent(<HTMLElement>dldtControl.children[0]);
    ...
    //对图表区增添内容
    let sxfxChart=this.diyChart(prototype[1]);
    return [sxfxControl, sxfxChart];
  }
 ...
 /*  private closeEvent(close:HTMLElement){
  ...
  private diyControl(control:HTMLElement,bounding):HTMLElement{
    ...
    content.appendChild(document.createTextNode('时序分析'));
